...
# Set the build target as needed e.g the "person_detection" example from the TFLite micro repo
export BUILD_TARGET=person_detection

# Build target
make --makefile=./tensorflow/lite/micro/tools/make/Makefile \
TARGET=sparkfun_edge ${BUILD_TARGET}_bin
...
test -f \
./tensorflow/lite/micro/tools/make/gen/sparkfun_edge_cortex-m4/bin/${BUILD_TARGET}.bin && \
...
# Before the next step, ensure you have conda (or pip/pip3)-installed the pycrypto and pyserial packages:
#
#       pip3 install pycrypto pyserial --user
#
# Otherwise you'll hit a "ModuleNotFoundError: No module named 'Crypto'" error

# Create signed binary
python3 ./tensorflow/lite/micro/tools/make/downloads/AmbiqSuite-Rel2.2.0/tools/apollo3_scripts/create_cust_image_blob.py \
--bin ./tensorflow/lite/micro/tools/make/gen/sparkfun_edge_cortex-m4/bin/${BUILD_TARGET}.bin \
--load-address 0xC000 \
--magic-num 0xCB \
...
# Attach to the Sparkfun Edge debug port over USB
screen ${DEVICENAME} 115200
#
# for (as_long_as_it_takes!)
#   reset the Sparkfun Edge
#   test
#   debug
#
# To exit the debug on Mac OS, use Ctrl + A then Ctrl + D
